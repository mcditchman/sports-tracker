...
## Current State
Soccer MVP complete — PR #1 (https://github.com/mcditchman/sports-tracker/pull/1): schema live on remote Supabase, cron-protected `/api/ingest` (daily 06:00 UTC via vercel.json). Frontend is now a browse → team drill-down instead of a single filter-everything page: `/` lists teams in a league with a live per-team data-coverage badge (matches with stats / total matches), and `/team/[teamId]` shows every stat type as a tile (season average, for/conceded) with one shared trend chart below driven by whichever tile is selected, filtered by window/venue/opponent. A `/team/[teamId]/player/[playerId]` page is designed (see `docs/superpowers/specs/2026-07-15-team-pages-design.md`) but not built — it's blocked on player-level stat ingestion, which doesn't exist yet. Season pinned to `2024` (2024-25 PL) per free-tier limits; cron continues backfilling stats daily. Before prod fully works: set `API_FOOTBALL_KEY`, `API_FOOTBALL_SEASON=2024`, `CRON_SECRET` in Vercel env vars (values in local `.env.local`; Vercel CLI not installed/authenticated on this machine).
